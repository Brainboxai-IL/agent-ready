...
export function generateFiles(scan: ProjectScan, score: ReadinessScore, force: boolean): GeneratedFile[] {
  ...
  function add(relativePath: string, content: string) {
...
function subdirClaudeFiles(scan: ProjectScan): Array<{ dir: string; content: string }> {
  if (!scan.monorepo.detected) return [];
...
function packageCommandLines(pkg: ProjectScan["packages"][number], projectPackageManager?: string): string[] {
...
function commandLines(scan: ProjectScan): string[] {
  ...
  if (!lines.length) lines.push("- No standard validation commands were detected. Add project-specific commands here.");
  return lines;
}
...
function entryPointLines(scan: ProjectScan): string[] {
  if (!scan.codeGraph.entryPoints.length) return ["- No entry points detected. Add known application, CLI, route, or library entry points here."];
  return scan.codeGraph.entryPoints.map((entry) => `- \`${entry.path}\` — ${entry.kind}; ${entry.reason}`);
}

function centralFileLines(scan: ProjectScan): string[] {
  if (!scan.codeGraph.centralFiles.length) return ["- No central files detected from imports yet."];
  return scan.codeGraph.centralFiles.slice(0, 15).map((file) => `- \`${file.path}\` — inbound: ${file.inbound}, outbound: ${file.outbound}`);
}

function importEdgeLines(scan: ProjectScan): string[] {
  if (!scan.codeGraph.importEdges.length) return ["- No resolved internal imports detected."];
  return scan.codeGraph.importEdges.slice(0, 40).map((edge) => `- \`${edge.from}\` → \`${edge.to}\``);
}

function externalImportLines(scan: ProjectScan): string[] {
  if (!scan.codeGraph.externalImports.length) return ["- No external imports detected in scanned source files."];
...
function generateAiIgnore(scan: ProjectScan): string {
  const base = [
  ...
  const add = (...patterns: string[]) => {
    for (const pattern of patterns) if (!base.includes(pattern)) base.push(pattern);
  };

  if (scan.packageManager || scan.languages.some((language) => ["TypeScript", "JavaScript", "Svelte"].includes(language))) add("node_modules/", "dist/", "build/", ".turbo/");
  if (scan.frameworks.includes("Next.js")) add(".next/");
  if (scan.frameworks.includes("Nuxt")) add(".nuxt/");
  if (scan.languages.includes("Python")) add(".venv/", "venv/", "__pycache__/");
  if (scan.languages.includes("Rust")) add("target/");
  if (scan.languages.includes("Go")) add("bin/");
  if (scan.languages.includes("C#")) add("bin/", "obj/");
  if (scan.languages.includes("PHP") || scan.frameworks.some((framework) => framework.includes("Composer"))) add("vendor/");
  for (const noisy of scan.noisyPaths) add(`${noisy}/`);
  return `${base.join("\n")}\n`;
}
...
function generateHooksReadme(scan: ProjectScan): string {
  return [
    "# Hook Templates",
    "",
...
function postEditHookLines(scan: ProjectScan): string[] {
  const lines = ["- Run or suggest the narrowest relevant validation command." ] ;
  if (scan.commands.lint?.length) lines.push(`- Lint candidate: \`${scan.commands.lint[0]}\``);
  if (scan.commands.typecheck?.length) lines.push(`- Typecheck candidate: \`${scan.commands.typecheck[0]}\``);
  if (scan.commands.test?.length) lines.push(`- Test candidate: \`${scan.commands.test[0]}\``);
  if (!scan.commands.lint?.length && !scan.commands.typecheck?.length && !scan.commands.test?.length) lines.push("- No validation commands detected yet; ask maintainers to document them.");
  return lines;
}

function generateRecommendations(scan: ProjectScan): string {
  const recs: string[] = [];
...
function lspRecommendations(scan: ProjectScan): string[] {
  const out: string[] = [];
  if (scan.traits.hasTypeScript || scan.languages.includes("JavaScript")) out.push("Install/enable TypeScript language server and ESLint language server for symbol-aware navigation.");
  if (scan.languages.includes("Python")) out.push("Enable Pyright or basedpyright for Python symbol navigation.");
  if (scan.languages.includes("Java")) out.push("Enable Java language server (jdtls). ");
  if (scan.languages.includes("C#")) out.push("Enable C# Dev Kit / Roslyn LSP.");
  if (scan.languages.includes("C/C++")) out.push("Enable clangd with a compile_commands.json where possible.");
  return out.length ? out : ["No specific LSP recommendation detected; add one for the primary language if available."];
}

function mcpRecommendations(scan: ProjectScan): string[] {
  const out = ["Use Context7/library-docs MCP for up-to-date framework/library APIs."];
  if (scan.databases.includes("Supabase")) out.push("Use Supabase MCP for schema/RLS inspection before DB changes.");
  if (scan.deployment.includes("GitHub Actions")) out.push("Expose CI logs/status through GitHub tooling/MCP if available.");
  if (scan.deployment.includes("Vercel")) out.push("Use Vercel tooling/MCP for deployments and production logs.");
  return out;
}

function hookRecommendations(scan: ProjectScan): string[] {
  const out = [
  ...
  if (scan.commands.lint?.length) out.push("Post-edit hook: suggest or run the narrowest lint command for changed workspace.");
  if (scan.commands.test?.length) out.push("Post-edit hook: suggest targeted tests related to changed files.");
  return out;
}
