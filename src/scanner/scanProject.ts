...
function normalizeBin(bin: PackageJson["bin"], pkgDir: string): string[] {
...
async function extractDescription(root: string, rootPackage?: PackageInfo): Promise<string | undefined> {
...
function firstProseParagraph(markdown: string): string | undefined {
  ...
  for (const raw of markdown.split(/\r?\n/)) {
    const line = raw.trim();
    if (/^(```|~~~)/.test(line)) { inFence = !inFence; continue; } // toggle fenced code block
    if (inFence) continue; // never treat code as prose (e.g. a ```bash language tag)
    if (!line) {
      if (paragraph.length) break; // blank line ends the first paragraph
      inCallout = false;
      continue;
    }
    if (line.startsWith(">")) {
      ...
      if (/^\[!\w+\]/i.test(content)) { inCallout = true; continue; }
      if (content) paragraph.push(content);
      continue;
    }
    inCallout = false;
    if (/^#{1,6}\s/.test(line)) { if (paragraph.length) break; else continue; } // heading
    if (/^(<!--|<\/?(p|div|img|a|br|picture|table|h[1-6]|center|sub|sup)\b)/i.test(line)) continue; // html block/badge wrappers
    if (/^([-*_])\1{2,}$/.test(line)) continue; // horizontal rule
    if (/^!\[/.test(line)) continue; // standalone image
    if (isBadgeOnlyLine(line)) continue; // shields.io badge rows
    paragraph.push(line);
  }
...
function isBadgeOnlyLine(line: string): boolean {
  if (/img\.shields\.io|badge|\bsvg\b/i.test(line) && /\]\(|<img/i.test(line)) return true;
...
async function detectEnvVars(root: string, relativeFiles: string[]): Promise<string[]> {
...
async function detectEntryPoints(root: string, packages: PackageInfo[], sourceSet: Set<string>, frameworks: string[]): Promise<EntryPoint[]> {
  ...
  for (const packageRoot of probeRoots) {
    ...
    for (const source of sourceSet) {
      if (packageRoot && !source.startsWith(packageRoot)) continue;
...
// full TOML parser.
async function detectManifestEntryPoints(root: string, sourceSet: Set<string>): Promise<EntryPoint[]> {
  ...
  if (pyproject) {
    for (const header of ["[project.scripts]", "[tool.poetry.scripts]"]) {
      for (const value of tomlStringValues(tomlSection(pyproject, header))) {
        ...
        if (resolved) entries.push({ path: resolved, kind: "Python entry", reason: "pyproject.toml console script" });
      }
    }
  }
  ...
  if (cargo) {
    for (const block of cargo.matchAll(/\[\[bin\]\]([\s\S]*?)(?=\n\s*\[|$)/g)) {
      ...
      if (sourceSet.has(file)) entries.push({ path: file, kind: "Rust binary", reason: "Cargo.toml [[bin]] path" });
    }
  }
...
function tomlStringValues(section: string): string[] {
  const out: string[] = [];
...
function isStdlibSpecifier(specifier: string, ext: string): boolean {
  if (/\.(tsx?|jsx?|mjs|cjs)$/.test(ext)) {
    const bare = specifier.startsWith("node:") ? specifier.slice(5) : specifier;
    return specifier.startsWith("node:") || NODE_BUILTINS.has(bare.split("/")[0]);
  }
  if (ext === ".py") return PYTHON_STDLIB.has(specifier.split(".")[0]);
  if (ext === ".rs") return RUST_STDLIB.has(specifier.split("::")[0]);
  return false;
}
...
async function detectHebrewOrRtl(files: string[]): Promise<boolean> {
  ...
  for (const file of textFiles) {
    const text = await readText(file);
    if (!text) continue;
    if (/dir=["']rtl["']|direction:\s*rtl/.test(text)) return true;
    if (uiExt.test(file) && /[\u0590-\u05FF]/.test(text)) return true;
  }
  return false;
}
